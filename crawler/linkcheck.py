...
# 掲載終了の確定に必要な連続 dead 回数（1日1回実行が前提。確度で猶予を変える）。
# 約24時間あけて連続 dead なら、瞬断ではなく実際の掲載終了とみなせる。
# ※ 実行頻度はこの猶予設計と対。crawl.yml の linkcheck ジョブを X投稿枠（JST 09時台）に
#   限定しており、頻度を変えるならこの定数も見直すこと。
# ※ 1日2回（約7時間間隔）から1日1回に減らしたため（2026-09-09）、掲載終了の確定は
#   404/410 で約1日、リダイレクトで約2日に伸びた。回数を維持したのは、間隔が空いたぶん
#   「同じ一時障害を2回続けて踏む」確率は下がる一方、サイト全体のメンテナンス等で
#   複数日にわたり dead を返すケースの誤確定は回数でしか防げないため。
