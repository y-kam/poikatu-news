...
# からのアクセスに対し「200 OK・案件0件」の応答を返す。HTTPエラーにならないため
# PoliteFetcher の再試行では拾えず、0件を空振りとみなしてここで取り直す。
# ※ 2026-08-23: 同一IPからの取り直しでは抜けられない（CI成功率17%→0%まで悪化）と判明し、
#   ちょびリッチ同様にCIから除外してローカル取得（tools/crawl_local.bat）へ移した。
#   この再試行は自宅回線での一時的な空振りに対する保険として残している。
EMPTY_RETRIES = 2
